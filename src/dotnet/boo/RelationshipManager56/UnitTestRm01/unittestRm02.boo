...
class TestCase01_OneToOne:
    ...
    [Test]    
    def checkOneToOne_XSingularApi_YNoApi():
    """
...
class UnittestCompareStuffUtility:
            ...
            |
             \      ,-----.
              `--->(  y2   )
                    `-----'
    """
        assert x1.getAllY() == [y1, y2] or x1.getAllY() == [y2, y1], "Actual situation {0} should be {1}" % (x1.getAllY(), [y1, y2])
        if yapi:
            assert y1.getX() == x1
            assert y2.getX() == x1
    ...
    def assertSituation04(x1 as duck, x2 as duck, y1 as duck, y2 as duck, yapi as bool):
        assert x1.getAllY() == [y1, y2] or x1.getAllY() == [y2, y1]
        if yapi:
            assert y2.getX() == x1
