# Diffs marked with word
# NEW

class RelationshipManagerOriginal2:
  """
  Slightly altered version of RelationshipManagerOriginal that has improved FindObjects().
  ...
  Differences from RelationshipManagerOriginal marked with # NEW
  """
  ...
  def RemoveRelationships(self, From, To, RelId=1):             # NEW (entire method replaced)
  ...
  def FindObjects(self, From=None, To=None, RelId=1):
      """ Specifying None as a parameter means 'any' """
      if From==None and To==None:                                                                   # NEW
          raise RuntimeError, "Either 'From' or 'To' has to be specified"                           # NEW
      resultlist = []
      havespecifiedallParams = lambda : (From<>None and To<>None and RelId<>None)
      match = lambda obj,list,index : obj==list[index] or obj==None
      for association in self.Relationships:
          #if match(From,association,0) and match(To,association,1) and RelId==association[2]:      # NEW
          if match(From,association,0) and match(To,association,1) and match(RelId,association,2):  # NEW
              if havespecifiedallParams():                                                          # NEW
                return True   # Found the only match, so stop looking                               # NEW
              if From==None:
                  resultlist.append(association[0])
              elif To==None:
                  resultlist.append(association[1])
              elif RelId==None:                        # NEW
                  resultlist.append(association[2])    # NEW
              #else:                                   # NEW
              #    resultlist.append(association)      # NEW
      if havespecifiedallParams():                     # NEW
        return False                                   # NEW
      return resultlist
